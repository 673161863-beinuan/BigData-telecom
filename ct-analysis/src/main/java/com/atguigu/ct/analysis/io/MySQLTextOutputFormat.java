...
public class MySQLTextOutputFormat extends OutputFormat<Text, Text> {
	...
	protected static class MySQLRecordWriter extends RecordWriter<Text, Text> {
		...
		public MySQLRecordWriter() {
			...
			/*
			 * PreparedStatement pstat = null; ResultSet rs = null;
			 * 
			 * try {
			 * 
			 * String queryUserSql = "select id, tel from ct_user"; pstat =
			 * connection.prepareStatement(queryUserSql); rs = pstat.executeQuery(); while
			 * (rs.next()) { Integer id = rs.getInt(1); String tel = rs.getString(2);
			 * userMap.put(tel, id); }
			 * 
			 * rs.close();
			 * 
			 * String queryDateSql = "select id, year, month, day from ct_date"; pstat =
			 * connection.prepareStatement(queryDateSql); rs = pstat.executeQuery(); while
			 * (rs.next()) { Integer id = rs.getInt(1); String year = rs.getString(2);
			 * String month = rs.getString(3); if (month.length() == 1) { month = "0" +
			 * month; } String day = rs.getString(4); if (day.length() == 1) { day = "0" +
			 * day; } dateMap.put(year + month + day, id); }
			 * 
			 * } catch (Exception e) { e.printStackTrace(); } finally { if (rs != null) {
			 * try { rs.close(); } catch (SQLException e) { e.printStackTrace(); } } if
			 * (pstat != null) { try { pstat.close(); } catch (SQLException e) {
			 * e.printStackTrace(); } } }
			 */
		}
